...
func TestHTTPRoutesServerAppPageIncludesVirtualFolderDrilldown(t *testing.T) {
	...
	if !strings.Contains(string(response.GetBody()), `<title>Live TV (XC)</title>`) || !strings.Contains(string(response.GetBody()), `<h1>Live TV (XC)</h1>`) {
		t.Fatalf("expected the user app identity to be Live TV (XC)")
	}
...
func TestHTTPRoutesServerAppPageIncludesOrderedFavorites(t *testing.T) {
	...
	for _, want := range []string{
		...
		`data-favorites-action=\"guide\"`,
		`aria-label=\"Filter favorite channels\"`,
		`class=\"favorite-card-actions\"`,
		`class=\"favorite-remove\"`,
		`title=\"Remove from favorites\"`,
		`icon("heart-solid")`,
		`.favorites-empty { min-height:`,
